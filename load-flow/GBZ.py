...
c = [[0 for j in range(8)] for i in range(12)] #2차원 배열 선언

for i in range(0,pd.DataFrame(new_L).shape[0]): #row
    for j in range(0,pd.DataFrame(new_L).shape[1]): #col
        index = [k for k, sub in enumerate(new_L[i][j]) if sub and isinstance(sub[0], int)]
        index = ['Z' + str(item) for item in index]
        c[i][j] = index

# print(pd.DataFrame(c))
# pd.DataFrame(c).to_csv('./담당부하.csv')