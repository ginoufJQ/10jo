row = int(input("Max number of row line: "))    #행의 최대 전선 개수 입력 (간단계통도에서 7)
col = int(input("Max number of column line: ")) #열의 최대 전선 개수 입력 (간단계통도에서 4)

map_list = []  #빈 리스트 생성 #피더고, 전선이고, 개폐기고 하나하나 개별로 생각
               #후에 함수에 조건 적용하기 위해서 분기점도 하나의 좌표로 설정 -> row*2 + 1

for i in range(col*2 + 1):         #2차원 리스트 생성 #열의 개별 모든 개수 더하면 col*2 + 1
    second_dim = []
    for j in range(row*2 + 1):     #행의 개별 모든 개수 더하면 row*2
        third_dim = []
        for k in range(4):         #(x좌표, y좌표, 종류, 값)을 담고있는 4차원 리스트(??) 생성 후 초기값 0대입
            third_dim.append(0)
        second_dim.append(third_dim)
    map_list.append(second_dim)

print(map_list)                    #내가 원하는 정보들을 입력하기 위한 3차원 배열 생성완료
